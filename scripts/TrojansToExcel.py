...
families_trojan = ['danabot', 'ramnit', 'tinba', 'darkcomet', 'upatre', 'phorphiex', 'agenttesla','buer', 'emotet', 'trickbot', 'icedid', 'ursnif', 'gozi_ifsb', 'qackbot', 'zloader', 'dridex', 'gootkit', 'bazarloader', 'bazarbackdoor', 'smokeloader', 'qakbot','systembc', 'guloader', 'oski', 'lokibot', 'vidar', 'cryptobot', 'evilgnome', 'raccoon', 'redline', 'servhelper', 'elysiumstealer', 'plugx', 'glupteba', 'jupyter', 'asyncrat', 'betabot', 'vjw0rm', 'alienbot', 'fickerstealer', 'cerberus', 'nanocore', 'formbook', 'bitrat', 'njrat']
def main():

    #Insert private Key
    key = " "

    for family in families_trojan:
        print(family)
        url = 'https://api.tria.ge/v0/search?query=family:'+family
        print(url)
        headers = {"Authorization": "Bearer " + key}
        try:
            #Request data of the last sumbitted samples. It retrieves 20 samples IDs or so.
            r = requests.get(url, headers=headers).json()

            i=0
            for item in r['data']:
                excel_final = load_workbook("./trojans.xlsx")
                ...
                with open('./sample_ids.txt') as f:
                    if (item['id'] in f.read()):
                        found_id = 0
                        print(item['id']+" Already in excel sheet")
                    else:
                        found_id = 1
                ...
                if(found_id == 1):
                    ...
                    response = requests.get('https://api.tria.ge/v0/samples/'+item['id']+'/overview.json', headers=headers).json()
